...
def get_junctions(jloc, joff, scale_factor, topk=300, th=0):
    height, width = jloc.size(1), jloc.size(2)
    jloc = jloc.reshape(-1)
    joff = joff.reshape(2, -1)

    scores, index = torch.topk(jloc, k=topk)
    r = torch.div(index, width, rounding_mode='floor').float() + torch.gather(joff[0], 0, index) + 0.5
    c = (index % width).float() + torch.gather(joff[1], 0, index) + 0.5
...
class LineSegDetector(nn.Module):
    def __init__(
        self, n_class, dim_loi=128, dim_fc=1024, n_pts0=32, n_pts1=16, d_max=5,
        ...
        if hour_glass_params is None:
            self.feat_extractor = build_hourglass(is_junction=True, is_line=True)
        else:
            self.feat_extractor = build_hourglass(
                is_junction=True, is_line=True,
    ...
    def forward(self, xs, targets=None, scale_factor=None, mode=None, T_juncs=0.008, T_lines=0.25):
        if mode is None:
        ...
        if is_training:
            ...
            return self.forward_eval(xs, scale_factor, T_juncs, T_lines)
    
    def forward_train(self, xs, targets):
        scale_factor = targets[0]["afm_scale_factor"]
        ...
        jloc_gt = torch.stack([target["jloc_gt"] for target in targets])
        joff_gt = torch.stack([target["joff_gt"] for target in targets])
        ...
        for output in outputs:
            ...
            loss_dict['loss_joff'] += joff_loss(joff_pred, joff_gt, jloc_gt) / len(outputs)
        ...
        for i, (md_pred_per_im, dis_pred_per_im, res_pred_per_im, target) in enumerate(
            ...
            for scale in [-1.0,0.0,1.0]:
                _ = self.proposal_lines(
                    md_pred_per_im, (dis_pred_per_im+scale*res_pred_per_im) * self.d_max, scale_factor
                ).view(-1, 4)
                lines_pred.append(_)
            lines_pred = torch.cat(lines_pred)
            juncs_pred, _ = get_junctions(
                non_maximum_suppression(jloc_pred[i]), joff_pred[i], scale_factor,
            ...
            hs_logits, hs_widths = self.pooling(loi_features[i], lines_for_train / scale_factor)
            ...
            # extras.append([lines_selected, labels_class_selected, labels_width_selected, lines_for_train, labels_class_for_train, hs_logits])

        return loss_dict #, extras
    
    def forward_eval(self, xs, scale_factor, T_juncs, T_lines):
        assert xs.shape[0]==1
        ...
        juncs_pred, _ = get_junctions(
            jloc_pred_nms, joff_pred[0], scale_factor,
        ...
        hs_logits, hs_widths = self.pooling(loi_features[0], lines_adjusted / scale_factor)

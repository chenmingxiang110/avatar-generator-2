...
class LineSegDetector(nn.Module):
    def __init__(
        self, n_class, dim_loi=128, dim_fc=1024, n_pts0=32, n_pts1=3, span_1=3, d_max=5,
        ...
        self.line_feat_extractor = nn.Sequential(
            nn.Conv2d(self.dim_loi, self.dim_loi, kernel_size=(4,3), stride=2, padding=(1,0)),
            nn.BatchNorm2d(self.dim_loi),
            nn.LeakyReLU(0.2, inplace=True),
        )
        
        self.fc_class = nn.Sequential(
            nn.Linear(self.dim_loi * (n_pts0//2), self.dim_fc),
            nn.ReLU(inplace=True),
            nn.Linear(self.dim_fc, self.dim_fc),
            nn.ReLU(inplace=True),
            nn.Linear(self.dim_fc, self.n_class),
    ...
    def pooling(self, features_per_image, lines_per_im):
        ...
        _features_per_line = self.line_feat_extractor(xp)
        _n_line = _features_per_line.shape[0]
        features_per_line = torch.reshape(_features_per_line, (_n_line, -1))

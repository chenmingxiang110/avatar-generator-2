...
class PSPUpsample(nn.Module):
    ...
    def forward(self, x):
        p = F.interpolate(input=x, scale_factor=2, mode='bilinear')
        return self.conv(p)
...
class PSPNet(nn.Module):
    def __init__(self, n_classes=18, sizes=(1, 2, 3, 6), psp_size=2048, deep_features_size=1024, backend='resnet34',
                 pretrained=False):
        super().__init__()
        self.feats = backend_dict[backend](pretrained)
    ...
    def forward(self, x, output_features=False, is_dropout=True):
        f, class_f = self.feats(x) 
        p = self.psp(f)
        if is_dropout: p = self.drop_1(p)

        p1 = self.up_1(p)
        if is_dropout: p1 = self.drop_2(p1)

        p2 = self.up_2(p1)
        if is_dropout: p2 = self.drop_2(p2)

        p3 = self.up_3(p2)
        if is_dropout: p3 = self.drop_2(p3)
        ...
        if output_features:
            return self.final(p3), p, p1, p2, p3
        else:
            return self.final(p3)